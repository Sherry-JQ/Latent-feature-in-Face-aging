...
def LoadDataset(name, root, batch_size, split, shuffle=True, resize_size=64,style=None, attr=None):
    # if name == 'mnist':
    #     if split == 'train':
    ...
    if name == 'face':
        ...
        if split == 'train':
            return LoadFace(root, style=style, split='train', batch_size=batch_size, resize_size=resize_size, shuffle=shuffle)
        elif split == 'test':
            return LoadFace(root, style=style, split='test', batch_size=batch_size,resize_size=resize_size, shuffle=False)
...
def LoadFace(data_root, batch_size=32, split='train', style='photo',resize_size=64, attr=None,
             shuffle=True, load_first_n=None):  # style -- a1,a2,a3
    if not os.path.exists(data_root):
        os.makedirs(data_root)

    key = '/'.join([split, style])
    UTKF_dataset = Face(data_root, key, split,resize_size,load_first_n)
...
class Face(Dataset):
    def __init__(self, root, key,split, resize_size,load_first_n=None):
